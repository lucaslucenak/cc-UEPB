...
if __name__ == '__main__':
    ...
    draw_pixel(p1[0], p1[1])

    lenght = abs(p2[0] - p1[0])
    if abs(p2[1] - p1[1] > lenght):
        lenght = abs(p2[1] - p1[0])

    x_incremento = (p2[0] - p1[0]) / lenght
    y_incremento = (p2[1] - p1[1]) / lenght

    actual_pixel_x = p1[0]
    actual_pixel_y = p1[1]

    mpl.plot(round(actual_pixel_x), round(actual_pixel_y), 'or')
    ...
    while True:
        ...
        while actual_pixel_x < p2[0]:
            actual_pixel_x += x_incremento
            actual_pixel_y += y_incremento

            draw_pixel(actual_pixel_x, actual_pixel_y)
            mpl.plot(round(actual_pixel_x), round(actual_pixel_y), 'or')
